...
import logging
import pygame
from multiagent_envs.officeworld.rgb_render import render_grid, add_text
...
class OfficeWorld(gym.Env):
    ...
    def __init__(self, rng=None, target=[4], step_cost=1.0, invalid_action_cost=10, terminal_reward=100):
    ...
    def render(self, screen_size=800, wait_time=0, mode='human'):
        ...
        if(mode=="human"):    
            ...
            office_grid = render_grid(input_string, screen_size, wait_time=wait_time)

            rem = "Tasks Remaining: "
            for i,e in enumerate(list(self.target)):
                if(i<3):
                    rem  += f"{TASKS[e]}, " 
                else:
                    rem += f"and {len(self.target)-2} more"

            ntasks  = f"Num Tasks Completed: {self.state.facts.count(1)}"
            
            # Create the display


            title = add_text("Office World", "title", screen_size)
            tasks_rem = add_text(rem, "body", screen_size)
            num_tasks = add_text(ntasks, "body", screen_size)
            screen.blit(office_grid, (screen_size*0.1, screen_size*0.1))
            screen.blit(title, (screen_size*0.21, screen_size*0.05))
            screen.blit(tasks_rem, (screen_size*0.025, screen_size*0.92))
            screen.blit(num_tasks,(screen_size*0.025, screen_size*0.88)) 
            pygame.display.flip()
...
if __name__ == "__main__":
    ...
    while i<=10000:
        ...
        env.render(mode="rgb", wait_time=200)
