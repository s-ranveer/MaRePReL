...
from utils.csp import sat_up, NaryCSP, Constraint, ac_search_solver, is_constraint
from utils.logic import FolKB, conjuncts, unify_mm, associate, SAT_plan, cdcl_satisfiable
from utils.search import Node
from utils.utils import Expr, expr, first
...
def taxi_domain():
    initial_state = 'At(Taxi, Garage) & At(Alice, LocationA) & At(Bob, LocationB) & Passenger(Alice) & Passenger(Bob) & Free(Taxi) & ~InTaxi(Alice) & ~InTaxi(Bob)'

    goal_state = 'At(Alice, Destination) & At(Bob, Destination)'

    move_action = Action('Move(taxi, from_loc, to_loc)',
                         precond='At(taxi, from_loc) & Free(taxi)',
                         effect='At(taxi, to_loc) & ~At(taxi, from_loc)',
                         domain='Taxi(taxi) & Location(from_loc) & Location(to_loc) & State(Free)')

    pickup_action = Action('PickUp(taxi, person, from_loc)',
                           precond='At(taxi, from_loc) & At(person, from_loc) & Passenger(person) & Free(taxi)',
                           effect='InTaxi(person) & ~At(person, from_loc) & ~Free(taxi)',
                           domain='Taxi(taxi) & Person(person) & Location(from_loc)')

    dropoff_action = Action('DropOff(taxi, person, to_loc)',
                            precond='InTaxi(person) & At(taxi, to_loc) & ~Free(Taxi)',
                            effect='At(person, to_loc) & ~InTaxi(person) & Free(Taxi)',
                            domain='Taxi(taxi) & Person(person) & Location(to_loc)')

    return PlanningProblem(initial=initial_state,
                           goals=goal_state,
                           actions=[move_action, pickup_action, dropoff_action],
                           domain='Taxi(Taxi) & Location(Garage) & Location(LocationA) & Location(LocationB) & Location(Destination) & Person(Alice) & Person(Bob)')
...
class RealWorldPlanningProblem(PlanningProblem):
    ...
    def refinements(self, library):  # refinements may be (multiple) HLA themselves ...
        ...
        'effect': [
            ['At(SFO) & ~At(Home)'],
            ['At(SFO) & ~At(Home)'],
            ['At(SFOLongTermP       arking) & ~At(Home)'],
