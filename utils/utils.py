...
def create_gif(save_dir=None,save_name="rl.gif",dir_name="imgs/",time_per_frame=10):
    fileNames = os.listdir(dir_name)
    if(not save_dir):
        save_dir=Path(dir_name).parent
    
    paths = [dir_name+"/"+fileName for fileName in fileNames]
    ...
    frames[0].save("{}/{}".format(str(save_dir),save_name), format='GIF',
                append_images=frames[1:], save_all=True, duration=len(paths)*time_per_frame, loop=0)
